...
//						$(".ui-dialog-titlebar-close").remove();
					...
					$("#eventContent").html('<div>결제수단 : '+select+'</div> <div>날짜 : '+event.payDate.substring(0,10)+'</div> <div> 지 출 명 : '+event.payName+'</div> <div>금  액 : '+parseInt(event.amount).toLocaleString()+'원</div> <div>영 수 증 : '+receiptName+'</div><div>'+receiptLink+'</div>');
						$( "#event" ).dialog( 
	                            { title:  event.payName }
	                        );
